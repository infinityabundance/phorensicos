...
| **8** | Immutable store generations + demand-driven port queue + explicit service generation binding | **core done** — `StoreGeneration`/`GenerationLedger` with content-identity lineage and fail-closed verification; the generation-session court **materializes a generation's exact runtime index** (`Artifacts(dispatcher) == Artifacts(gen)`) and serves every port it binds — generation 1 serves 14 ports from 7 objects, distinguishing both `strspn` identities — while a bound session refuses any port its generation does not bind; a runtime miss emits a bounded, non-blocking `PortDemand`; deterministic integer demand ranking with a recorded breakdown (`docs/STORE_GENERATIONS.md`) |
...
K by the generation ledger, the generation-bound service, the committed
`strspn` child generation (`8e6fafec…`,
`phost/evidence/phorport/autonomy/libc-strspn-c-locale-u64-v1/store_generation.json`),
and its consumption (the generation session materializes the exact index and serves
14 ports / 7 objects); L below).
