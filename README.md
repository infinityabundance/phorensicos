...
| Docker (Gate L) | `docker compose run --rm host` / `kernel` reproduce the tests, the store, the courts, and the QEMU boot — verified from a clean checkout: **host 74 verifier blocks, 0 failures; kernel 13/13 boot checks, 5/5 evidence artifacts byte-reproducible** |
