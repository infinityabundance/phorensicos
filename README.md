...
| Corrected provenance | `strspn` is an ISO C surface (C90 4.11.5.4; C99+ 7.21.5.4); the committed baseline historically recorded it as `posix:strspn:c-locale:u64:v1` on a mistaken claim. That record, its `PortSpecId` and its evidence are preserved; the corrected successor `libc:strspn:c-locale:u64:v1` is requalified and sealed under its own identity (`AutonomousV1`, 13/13 obligations) and published additively as **immutable store generation 1**. The generation is then **consumed**: `phost port generation-session` materializes its exact runtime index and serves **14 ports from 7 objects**, distinguishing `posix:strspn` (historical object) from `libc:strspn` (successor object), while the committed v1 index and the `d219be2c…` session verdict are unchanged; see `docs/CONTRACT_PROVENANCE_MIGRATION.md`, `docs/STORE_GENERATIONS.md`, `./verify_supersession.sh`, `./verify_generation_session.sh` |
...
| Docker (Gate L) | `docker compose run --rm host` / `kernel` reproduce the tests, the store, the courts, and the QEMU boot — verified from a clean checkout: **host 75 verifier blocks, 0 failures; kernel 13/13 boot checks, 5/5 evidence artifacts byte-reproducible** |
| Release lock | the tag `autonomous-porting-v1` binds the revision, and `foundry/releases/autonomous-porting-v1.json` locks the empirical identities and tables (store, session, provenance correction, successor seal, generation, generation session) **derived from committed evidence**; `./scripts/release_lock.sh --check …` (run by `ci_host.sh`) fails if the lock drifts from the courts |
